...
// JoinCondition represents a table join condition
...
// OrCondition represents a group of conditions that should be joined with OR
...
// AddJoin adds a table join condition
func (f *BaseFilter) AddJoin(joinType JoinType, table string, condition string, parameters ...any) {
	f.Joins = append(f.Joins, JoinCondition{
		Type:       joinType,
		Table:      table,
		Condition:  condition,
...
// AddOrConditions adds a set of conditions that will be combined with OR
func (f *BaseFilter) AddOrConditions(conditions ...FilterCondition) {
	if len(conditions) > 0 {
		f.OrConditions = append(f.OrConditions, OrCondition{
